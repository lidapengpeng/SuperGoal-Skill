...
It:   Objective: 改进 SAM 3 在遥感影像中的建筑提取效果。
      ...
      Plan (provisional): DESIGN → FINAL；实现子目标由设计阶段导出
      Assumptions: [low] labels 可用；[HIGH] GPU budget 足够跑 3 次。
      Tier: high-risk · design cost: ~2 survey Q, 2 debate rounds
      Reply "go" to start.

You:  go

It:   [/goal created · PLAN.md (DESIGN+FINAL) + BRIEF.md written]
      [Research → Design draft → debate → inspection → derive SG1..SGn]
      [hard re-Agree if criterion/mechanism/budget moved]
...
- Stop hook 检查账本一致性，以及（当存在 `## Novelty` 时）mechanism
  contract 的**标签是否齐全**——不检查科学有效性；reviewer gates 和
